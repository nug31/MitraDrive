...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (role !== 'checker' && role !== 'admin') {
        alert('Akses ditolak. Halaman ini khusus untuk Checker.');
        window.location.href = 'index.html';
        return;
    }

    currentSession = session;
    document.getElementById('checkerName').textContent = fullName;
    document.getElementById('checkerEmail').textContent = session.user.email;
    ...
    actionForm.addEventListener('submit', async (e) => {
        ...
        try {
            const updatePayload = {
                status: statusType,
                catatan_checker: catatan || null,
                status_checker: statusType === 'ditolak' ? 'ditolak' : 'disetujui',
...
function updateStats() {
    const pending = allBookings.filter(b => b.status === 'menunggu_checker').length;
    const approved = allBookings.filter(b => ['menunggu_leader','menunggu_koordinator','disetujui','selesai'].includes(b.status)).length;
...
function filterAndRender() {
    ...
    filtered.forEach(booking => {
        ...
        let actionsHtml = '';
        if (booking.status === 'menunggu_checker') {
            actionsHtml = `
                <div class="action-buttons" style="display:flex; flex-direction:column; gap:4px;">
                    <div style="display:flex; gap:4px;">
                        <button class="btn-approve" data-id="${booking.id}" title="Setujui & Teruskan ke Leader">
        ...
        let catatanHtml = '';
        if (booking.catatan_checker) {
            catatanHtml = `<div class="catatan-leader-box" title="Catatan Checker"><strong>Catatan:</strong> ${booking.catatan_checker}</div>`;
        }
        ...
        if (approveBtn) approveBtn.addEventListener('click', () => openActionModal(booking.id, 'menunggu_leader'));
...
function openActionModal(id, type) {
    ...
    if (type === 'menunggu_leader') {
        title.textContent = 'Setujui Pengajuan';
        subtitle.textContent = 'Pengajuan akan diteruskan ke Direct Leader untuk persetujuan.';
        ...
        btnConfirm.textContent = 'Setujui & Teruskan';
    } else {
