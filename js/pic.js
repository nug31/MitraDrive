...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (role !== 'pic_peminjaman' && role !== 'admin') {
        alert('Akses ditolak. Halaman ini khusus untuk PIC Peminjaman.');
        window.location.href = 'index.html';
        return;
    }

    currentSession = session;
    document.getElementById('picName').textContent = fullName;
    document.getElementById('picEmail').textContent = session.user.email;
...
function filterAndRender() {
    ...
    filtered.forEach(booking => {
        ...
        if (booking.catatan_pic) {
            catatanHtml = `<div class="catatan-leader-box" title="Catatan PIC"><strong>Catatan PIC:</strong> ${booking.catatan_pic}</div>`;
        }
...
function openActionModal(id, type) {
    ...
    if (type === 'menunggu_checker') {
        ...
        btnConfirm.textContent = 'Verifikasi & Teruskan';
    } else {
...
function getStatusDisplay(status) {
    const map = {
        'menunggu_pic':         { badgeClass: 'badge-menunggu', statusIcon: 'bx-time-five', statusText: 'Menunggu PIC' },
...
function formatDateTime(dateTimeStr) {
    ...
    try {
        ...
        return date.toLocaleDateString('id-ID', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' }).replace(',', '');
